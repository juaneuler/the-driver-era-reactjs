...
const Checkout = () => {
  ...
  const manejarSubmit = async (data) => {
    const { nombre, apellido, email, telefono, domicilio, emailConfirmacion } =
      data;
    ...
    try {
      ...
      trackGTMEvents("purchase", {
        transaction_id: ordenRef.id, // ID real de Firestore
        value: total, // Valor total calculado
        currency: "USD",
      ...
      for (const producto of carrito) {
        ...
        if (typeof producto.stock === "object" && producto.talle) {
          ...
          nuevoStock[producto.talle] = Math.max(
            0,
            (nuevoStock[producto.talle] || 0) - producto.cantidad,
          );
          await updateDoc(productoRef, { stock: nuevoStock });
        } else if (typeof producto.stock === "number") {
          await updateDoc(productoRef, {
            stock: Math.max(0, producto.stock - producto.cantidad),
          });
        }
      }
            ...
            <form onSubmit={handleSubmit(manejarSubmit)}>
              ...
              <InputFormulario
                ...
                validation={{
                  ...
                  validate: (value) =>
                    value === email || "Los mails ingresados no coinciden",
                }}
              />
              <InputFormulario
                label="Teléfono"
